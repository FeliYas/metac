...
const columns = ['orden', 'banners', 'portada', 'path', 'titulo', 'subtitulo', 'descripcion', 'productos relacionados'];

// Definición de rutas dinámicas
const createRoute = route('industrias.store');
const updateRoute = (id) => route('industrias.update', { id });
const deleteRoute = (id) => route('industrias.destroy', { id });
const productosRoute = (id) => route('relacionados.dashboard', { id });
...
<template>
    <div>
        ...
        <DataTable :columns="columns" :data="industrias" :createRoute="createRoute" :updateRoute="updateRoute"
            :deleteRoute="deleteRoute" :productosRoute="productosRoute" recomendacion="600x600" />
    </div>
</template>
